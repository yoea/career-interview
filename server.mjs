...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/dashboard/stats') {
    ...
    try {
      ...
      // Daily array (all time, from first date to today)
      const allDates = Object.keys(dailyMap).sort()
      const firstDate = allDates.length ? allDates[0] : new Date(now - 29 * dayMs).toISOString().slice(0, 10)
      const totalDays = Math.floor((now - new Date(firstDate + 'T00:00:00').getTime()) / dayMs) + 1
      const daily = []
